    ...
    <footer className="bg-white border-t border-gray-200 mt-auto">
      <div className="max-w-7xl mx-auto px-4 py-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <h3 className="text-xs font-semibold text-gray-700 uppercase tracking-wider mb-2">About</h3>
            <p className="text-xs text-gray-500">Your trusted online store.</p>
          </div>
          <div>
            <h3 className="text-xs font-semibold text-gray-700 uppercase tracking-wider mb-2">Links</h3>
            <ul className="space-y-1">
              <li><button className="text-xs text-gray-500 hover:text-gray-700">Privacy</button></li>
              <li><button className="text-xs text-gray-500 hover:text-gray-700">Terms</button></li>
            </ul>
          </div>
          <div>
            <h3 className="text-xs font-semibold text-gray-700 uppercase tracking-wider mb-2">Contact</h3>
        ...
        <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-4">
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="text-xs text-gray-500">
              {filteredProducts.findIndex(p => p.id === selectedProduct.id) + 1} of {filteredProducts.length}
            </span>
            <button
            ...
            <div className="p-4">
              <div className="flex justify-between items-start mb-3">
                ...
                <span className={`text-xs px-2 py-1 rounded ${
                  selectedProduct.stock > 0 ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-500'
                }`}>
              ...
              <div className="border-t border-gray-100 pt-4 mb-4">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-gray-900">${selectedProduct.sell_price}</p>
                  </div>
                  <div>
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col">
        ...
        <main className="flex-1 max-w-7xl w-full mx-auto px-4 py-4">
          ...
          <h2 className="text-lg font-semibold mb-3 text-gray-800">Shopping Cart ({cartCount} items)</h2>
          ...
          {cart.length === 0 ? (
            ...
            <>
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full border border-gray-200 text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="border-b border-gray-200 p-2 text-left text-xs font-medium text-gray-500">Product</th>
                      <th className="border-b border-gray-200 p-2 text-left text-xs font-medium text-gray-500">Price</th>
                      <th className="border-b border-gray-200 p-2 text-left text-xs font-medium text-gray-500">Quantity</th>
                      <th className="border-b border-gray-200 p-2 text-left text-xs font-medium text-gray-500">Subtotal</th>
                  ...
                  <tbody>
                    {cart.map(item => (
                      <tr key={item.product.id} className="border-b border-gray-100">
                        <td className="p-2 text-sm text-gray-800">{item.product.name}</td>
                        <td className="p-2 text-sm">${item.product.sell_price}</td>
                        <td className="p-2">
                        ...
                        <td className="p-2 text-sm font-medium">${(item.product.sell_price * item.quantity).toFixed(2)}</td>
                        <td className="p-2">
                          <button 
                            onClick={() => removeFromCart(item.product.id)}
                            className="text-gray-400 hover:text-red-500"
                    ...
                    <tr className="bg-gray-50">
                      <td colSpan={3} className="p-2 text-right text-xs font-medium text-gray-600">Total:</td>
                      <td className="p-2 text-sm font-bold text-gray-900">${getCartTotal().toFixed(2)}</td>
                      <td></td>
                    </tr>
                  </tbody>
                </table>
              ...
              <div className="max-w-md mx-auto">
                <h3 className="text-sm font-semibold mb-3 text-gray-800">Customer Details</h3>
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 py-4">
        ...
        <div className="mb-4 space-y-2">
          ...
          {/* Filter Options */}
          <div className={`${showFilters ? 'block' : 'hidden'} md:block space-y-2 md:space-y-0 md:flex md:items-center md:gap-2`}>
            {/* Category Filter */}
            <select
          ...
          <div className="flex justify-between items-center text-xs text-gray-500">
            ...
            {(searchQuery || selectedCategory !== 'all') && (
              <span className="text-gray-400">
                {filteredProducts.length === 0 ? 'Try adjusting your filters' : ''}
              </span>
            )}
          </div>
        </div>
        ...
        {filteredProducts.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
            {filteredProducts.map(product => (
              <div key={product.id} className="border border-gray-200 rounded overflow-hidden hover:shadow-sm transition-shadow">
                <div className="aspect-square bg-gray-50 flex items-center justify-center relative group">
                  ...
                  >
                    <span className="bg-white text-gray-800 text-xs px-2 py-1 rounded border border-gray-200 shadow-sm">
                ...
                <div className="p-2">
                  <div className="flex items-center justify-between mb-1">
                    <h3 className="font-medium text-xs text-gray-800 line-clamp-1">{product.name}</h3>
                    <button
                      onClick={() => setSelectedProduct(product)}
                  ...
                  {product.category && (
                    <span className="text-[10px] text-gray-400 mb-1 block">{product.category}</span>
                  )}
                  
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-bold text-gray-900">${product.sell_price}</span>
